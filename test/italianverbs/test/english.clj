(ns italianverbs.test.english
  ...
  (:require
   ...
   [italianverbs.pos :refer [adjective animal
   ...
   [italianverbs.unify :as unify :refer [dissoc-paths get-in]]))
...
(def compiled (future (-> (compile-lex test-lexicon
...
(deftest two-entries
  ...
  (is (= (.size (get @compiled "buy")) 2))

  ;; both transitive and intransitive: check subject spec
  (is (= {:human true} (get-in (nth (get @compiled "buy") 0) [:synsem :sem :subj])))
  (is (= {:human true} (get-in (nth (get @compiled "buy") 1) [:synsem :sem :subj])))

  ;; transitive sense: check object spec.
  (is (or (= {:buyable true} (get-in (nth (get @compiled "buy") 0) [:synsem :sem :obj]))
          (= {:buyable true} (get-in (nth (get @compiled "buy") 1) [:synsem :sem :obj]))))

  ;; intransitive sense: check subject spec.
  (is (or (= :none (get-in (nth (get @compiled "buy") 0) [:synsem :sem :obj] :none))
          (= :none (get-in (nth (get @compiled "buy") 1) [:synsem :sem :obj] :none)))))
