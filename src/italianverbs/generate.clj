...
(defn head-by-comps [parent head comps]
  "Returns a lazy sequence of expressions generated by adjoining (head,comp_i) to parent, for all
   comp_i in comps."
;  (log/info (str "head-by-comps: " (fs/get-in parent '(:comment))))
  (if (not (empty? comps))
    (let [comp (first comps)
          ...
          comp-expand (expand comp-specification)]
...
(defn random-sentence []
  (first (take 1 (expand gram/s-present))))
...
;; move this to test:
(defn test1 []
  (head-by-comps gram/vp-present (lex/it "parlare")
                 (expand gram/np)))
